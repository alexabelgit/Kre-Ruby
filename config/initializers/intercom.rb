Rails.configuration.intercom = ActiveSupport::OrderedOptions.new
intercom = Rails.configuration.intercom
intercom.debug_enabled = ENV['DEBUG_INTERCOM'] || false
intercom.access_token = ENV['INTERCOM_ACCESS_TOKEN']
intercom.handle_rate_limit = true
...
IntercomRails.config do |config|
  ...
  #
  # config.user.exclude_if = Proc.new { |user| user != true_user }
  ...
  #
  config.company.current = Proc.new { current_user.store }
  ...
  config.company.custom_data = {
    :name => Proc.new { |store| store.name },
    :url  => Proc.new { |store| store.url }
  }
